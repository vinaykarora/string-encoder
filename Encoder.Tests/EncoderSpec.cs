...
namespace Service.Tests
{
    ...
    *****/
    public class EncoderSpec
    {
        ...
        {
            // Arrange
            IList<IReplaceService> replaceServices = new List<IReplaceService>
            {
                new VowelReplaceService(),
                new SpaceReplaceService(),
                new YReplaceService(),
                new ConsonantReplaceService(),
                new DefaultReplaceService()
            };
            ...
            // Assert
            Assert.Equal(expected, new EncoderProcessor(replaceServices).Encode(toEncode));
            stopWatch.Stop();
            var totalMemory = GC.GetTotalMemory(false) * 0.000001;
            Console.WriteLine("Memory (MB): " + totalMemory);
            Console.WriteLine("Runtime: " + stopWatch.ElapsedMilliseconds + "ms");
        }

        [Fact]
        public void Encoder_InvalidInput_ShouldThrowArgumentException()
        {
            // Arrange
            IList<IReplaceService> replaceServices = new List<IReplaceService>
            {
            ...
            // Assert
            Assert.Throws<ArgumentException>(() => new EncoderProcessor(replaceServices).Encode(null));
            stopWatch.Stop();
            var totalMemory = GC.GetTotalMemory(false) * 0.000001;
            Console.WriteLine("Memory (MB): " + totalMemory);
            Console.WriteLine("Runtime: " + stopWatch.ElapsedMilliseconds + "ms");
        ...
        [Theory]
        [InlineData(@"The one from the village, FN2187")]
        public void Encoder_InvalidInput_ShouldInvalidOperationException(string toEncode)
        {
            // Arrange
            IList<IReplaceService> replaceServices = new List<IReplaceService>
            {
            ...
            // Assert
            Assert.Throws<InvalidOperationException>(() => new EncoderProcessor(replaceServices).Encode(toEncode));
            stopWatch.Stop();
            var totalMemory = GC.GetTotalMemory(false) * 0.000001;
            Console.WriteLine("Memory (MB): " + totalMemory);
            Console.WriteLine("Runtime: " + stopWatch.ElapsedMilliseconds + "ms");
        ...
        [Theory]
        [InlineData(@"The one from the village, FN2187")]
        public void Encoder_InvalidInput_ShouldArgumentNullException(string toEncode)
        {
            // Arrange
            IList<IReplaceService> replaceServices = null;
            ...
            // Assert
            Assert.Throws<ArgumentNullException>(() => new EncoderProcessor(replaceServices).Encode(toEncode));
            stopWatch.Stop();
            var totalMemory = GC.GetTotalMemory(false) * 0.000001;
            Console.WriteLine("Memory (MB): " + totalMemory);
            Console.WriteLine("Runtime: " + stopWatch.ElapsedMilliseconds + "ms");
